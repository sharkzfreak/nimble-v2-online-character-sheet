...
import { BERSERKER_SUBCLASS_FEATURES, CHEAT_SUBCLASS_FEATURES, COMMANDER_SUBCLASS_FEATURES, HUNTER_SUBCLASS_FEATURES, MAGE_SUBCLASS_FEATURES, VENGEANCE_SUBCLASS_FEATURES, REFUGE_SUBCLASS_FEATURES, SHADOWMANCER_SUBCLASS_FEATURES, SHEPHERD_SUBCLASS_FEATURES } from "@/config/subclassFeatures";
...
const ClassDetail = () => {
  ...
  const getSubclassFeaturesByLevel = () => {
    ...
    if (className === 'Oathsworn') {
      const oathswornFeatures = [...VENGEANCE_SUBCLASS_FEATURES, ...REFUGE_SUBCLASS_FEATURES];
      oathswornFeatures.forEach(subclassFeature => {
        const level = subclassFeature.level;
        if (!grouped[level]) {
          grouped[level] = {};
        }
    ...
    if (className === 'Shadowmancer') {
      SHADOWMANCER_SUBCLASS_FEATURES.forEach(subclassFeature => {
        const level = subclassFeature.level;
        if (!grouped[level]) {
          grouped[level] = {};
        }
    ...
    if (className === 'Shepherd') {
      SHEPHERD_SUBCLASS_FEATURES.forEach(subclassFeature => {
        const level = subclassFeature.level;
        if (!grouped[level]) {
          grouped[level] = {};
        }
